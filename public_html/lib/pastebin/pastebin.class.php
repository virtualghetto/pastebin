	...
	*/
	function getPost($pid)
	{
		...
		if ($post)
		{
			...
			$post['spamurl']=$this->conf['this_script']."?report=$pid";
	...
	{
		$this->db->addAbusePost($pid, $this->conf['subdomain'], $msg);
	}

	function getAbusePosts()
	{
		//get raw db info
		$posts=$this->db->getAbusePostSummary($this->conf['subdomain']);
	...
	function getAbusePost($pid)
	{
		$post=$this->db->getAbusePost($pid, $this->conf['subdomain']);
		return $post;
	}
