...
class Diff
{
	...
	function __construct( $f1 , $f2 , $show_equal = 0 )
	{
		...
		$hit1	   = array() ;				  # hit in left
		$hit2	   = array() ;				  # hit in right
		$out = "";
		$stop = 0;
			  ...
			  )
		{
			...
			*/
			if ( (isset($f1[$c1]) && isset($f2[$c2]) ) && ( trim( $f1[$c1] ) == trim ( $f2[$c2])  ) )
			{
				/**
					  ...
					  )
				{
					...
					*/
					...
					else
					{
						...
						*/
						if (!isset($hit1[ ($c1 + $s1) . "_" . ($c2)])) // ( $hit1[ ($c1 + $s1) . "_" . ($c2) ] != 1 )
						{
							/**
					...
					*/
					...
					else
					{
						...
						*/
						if (!isset($hit2[ ($c1) . "_" . ( $c2 + $s2) ])) //( $hit2[ ($c1) . "_" . ( $c2 + $s2) ] != 1 )
						{
							/**
